...
export const updateVideoAsset = async (muxUploadIdOrAssetId: string) => {
	const [existingVideo] = await db
		.select()
		.from(videos)
		.where(or(eq(videos.muxUploadId, muxUploadIdOrAssetId), eq(videos.muxAssetId, muxUploadIdOrAssetId)))
		.limit(1);
	...
	try {
		upload = await mux.video.uploads.retrieve(muxUploadIdOrAssetId);
	} catch {}

	const assetIdFromUpload = upload?.asset_id;
	const assetId = assetIdFromUpload || existingVideo.muxAssetId || muxUploadIdOrAssetId;

	if (assetIdFromUpload) {
		try {
			asset = await mux.video.assets.retrieve(assetIdFromUpload);
		} catch {}
	}
	...
	if (!muxPlaybackId) {
		const playback = await mux.video.assets.createPlaybackId(asset.id, {
			policy: 'public',
		});
	...
	if (!muxPlaybackId) {
		throw new TRPCError({ code: 'BAD_REQUEST', message: 'Mux playback id not found!' });
	}
